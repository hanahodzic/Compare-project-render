...
window.onload = function() {
  ...
  function findTwoSimilarProducts(products, query) {
    ...
    // Match by title
    let strongMatch = filteredProducts.filter(product =>
        product.title.toLowerCase().includes(queryLower)
    );

    // If no title match, fallback to Levenshtein
    if (strongMatch.length === 0) {
        strongMatch = filteredProducts.map(product => ({
            product,
...
document.getElementById("search-button").addEventListener("click", function () {
    ...
    fetch('/api/products')
        ...
        .then(products => {
            const results = findTwoSimilarProducts(products, query);
