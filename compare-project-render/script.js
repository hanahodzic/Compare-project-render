...
window.onload = function() {
  ...
  function findTwoSimilarProducts(products, query) {
    ...
    // Step 1: Strong title match
    let strongMatch = products.filter(product =>
        product.title.toLowerCase().includes(queryLower)
    );

    // Step 2: If no title match, find similar by Levenshtein
    if (strongMatch.length === 0) {
        strongMatch = products.map(product => ({
            product,
            distance: levenshteinDistance(product.title.toLowerCase(), queryLower)
        }))
    ...
    // Step 3: Final distance ranking within top matches
    const ranked = strongMatch.map(product => ({
        product,
        distance: levenshteinDistance(product.title.toLowerCase(), queryLower)
    }))
    .sort((a, b) => a.distance - b.distance);

    return ranked.slice(0, 2).map(p => p.product);
}
