...
function findTwoSimilarProducts(products, query) {
    ...
    if (filtered.length === 0) {
        const bestCategory = findBestCategoryMatch(products.map(p => p.category), query);
        filtered = products.filter(p => p.category === bestCategory);
    }

    const ranked = filtered.map(p => ({
        product: p,
        similarity: levenshteinDistance(p.title.toLowerCase(), query)
...
function findBestCategoryMatch(categories, query) {
    const ranked = categories.map(cat => ({
        category: cat,
        similarity: levenshteinDistance(cat.toLowerCase(), query)
    })).sort((a, b) => a.similarity - b.similarity);

    return ranked[0]?.category || categories[0];
}
